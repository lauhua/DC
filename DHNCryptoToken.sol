...
contract StandardToken is ERC20, BasicToken {
    ...
    mapping (address => uint256) public timelockDeadline;
    mapping (address => uint256) public timelockAmount;

    event FreezeAccount(address target);
    event UnfreezeAccount(address target);
    event TimelockAccount(address target, uint256 deadline, uint256 amount);
    event UnlockAccount(address target);
...
contract DHNCryptoToken is PausableToken {
    string public name = "DHN Crypto Token";
    string public symbol = "DCT";
    uint public decimals = 18;
    uint public INITIAL_SUPPLY = 2600000000 * 10 ** uint(decimals);

    function DHNCryptoToken() public{
